...
//we are using useref which is a client hooks and hence use client 
export default function WizardModel(props) {
  const { nodes, materials } = useGLTF('/models/wizard-transformed.glb')
  const modelRef = useRef();
  //It allows you to execute code on every rendered frame, 
  //State: An object containing the current state of the Three.js scene, including the camera, scene, and renderer.
// Delta: The time elapsed since the last frame, measured in milliseconds. 
  useFrame((state,delta,xrFrame)=>{
    //no matter how much value goes up the sin will just start from -1 and go to 1 only for any value so it will give a up and down movement like feel
      modelRef.current.position.y = -1.5 + Math.sin(state.clock.elapsedTime)*0.15
  })
  return (
    <group {...props} dispose={null} position={[0.1,-1.5,0]} scale={[0.06,0.06,0.06]}
    rotation={[0.25,0,0]} ref={modelRef}>
      <mesh
        castShadow
        receiveShadow
